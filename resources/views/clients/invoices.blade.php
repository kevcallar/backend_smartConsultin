        ...
        <table class="min-w-full bg-zinc-700 divide-y divide-zinc-500">
            <thead class="bg-zinc-400">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider"><span tabindex="0" data-toggle="tooltip" title="Cambia el estado del documento">Estado
                    </span></th>
            ...
            <tbody class="divide-y divide-zinc-500">
                @foreach ($invoices as $invoice )
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-white">{{ $invoice->created_at->format('d-m-Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-white">{{ $invoice->type }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                              <div class="form-check form-switch">
                                <span tabindex="0" data-toggle="tooltip" title="Cambia el estado del documento">
                                    <input class="form-check-input checked:bg-teal-500 checked:hover:text-teal-500 checked:focus:bg-teal-500 checked:focus:border-teal-500 focus:ring-teal-500 dark:focus:ring-teal-600 dark:focus:ring-offset-teal-800" type="checkbox" id="InvoiceToggle" name="state" value="1" data-invoice-id="{{ $invoice->id }}" @if($invoice->estado == 1) checked @endif>
                                </span>
                              </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                          <a href="#" data-bs-toggle="modal" data-bs-target="#InvoiceModal{{ $invoice->id }}" class="text-teal-500 hover:text-teal-600">Ver</a>
                        </td> 
                      </tr>
                      @endforeach
                    </tbody>
...
@endif
                    ...
                    <livewire:invoice-modal :invoices="$client->invoices" /> 
